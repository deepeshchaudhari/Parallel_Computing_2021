...
int main( int argc, char *argv[])
{
  ...
  for (int step = 0; step < steps; step++)
  {
      ...
      //  =================== Packing data in up, left, right and down array ======================================= 
      for (int i=0; i<array_of_sizes; i++){
       ...
       // ================= Sending left, right, up and down to parrallel processes ==============================
     ...
     //==================== recieving up, down, left and right from parallel processes ====================
      ...
      // =========== solve Halo center and initializing up_data ,down_data, left_data , and right_data ===================
     ...
     double right_data;
            ...
            for (int j=0;j<array_of_sizes;j++){
                    ...
                    if ((i>0 && i<array_of_sizes-1) && (j>0 && j<array_of_sizes-1)) {
                          ...
                          temp_data[i][j] = (up_data + down_data + left_data + right_data)/4;
                            ...
                            if (i==0){
                                // myrank has no up, get from recieved array
                                up_data = recv_up[j];
                               }
                            else{
                                up_data = data[i-1][j];
                            ...
                            if (i == array_of_sizes-1){
                                // myrank has no down, get from recieved array
                            ...
                            if (j==array_of_sizes-1){
                                // myrank has no right, get from recieved array
                            ...
                            if (j==0){
                                  // myrank has no left, get from recieved array
      ...
      // update array after computation and do further steps  
      for (int i = 0; i < array_of_sizes; i++)
          {
              for (int j = 0; j < array_of_sizes; j++)
              {
...
// ================  free all dynamically allocated variable ========================
free(vector_data);
free(single_data);
free(data);
